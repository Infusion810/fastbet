...
const BetSection = ({
  ...
  useEffect(() => {
    if (selectedBet?.type && stakeValue) {
      ...
      console.log('Type:', selectedBet.type);
      console.log('Odds:', selectedBet.odds);
      console.log('Stake:', stakeValue);
      console.log('Rate:', selectedBet.rate);
      console.log('Calculated Profit:', calculatedProfit);
      console.log('=====================');
    } else {
      setProfit(0);
    }
  }, [selectedBet, stakeValue, calculateProfit, setProfit]);
          ...
          <tbody>
            {myBets && myBets.length > 0 ? (
              myBets
                ...
                .reverse()
                .map((bet, betIdx) => {
                  const isOverMarketBet = bet.marketType === 'overMarket';
                  const exposure = bet.currentExposure || bet.exposure || 0;
                  ...
                  return (
                    <tr key={betIdx}>
                      ...
                      <td>{bet.odds || "-"}</td>
                      <td>{isOverMarketBet ? (bet.rate || bet.runValue || "-") : (bet.odds || "-")}</td>
                      <td>{bet.stake || "0.00"}</td>
                      <td style={{ color: parseFloat(bet.teamAProfit) >= 0 ? 'green' : 'red' }}>
                        {isOverMarketBet ? 
                          (bet.type.toLowerCase() === 'yes' ? bet.teamAProfit : -bet.stake).toFixed(2) :
                          bet.teamAProfit || "0.00"}
                      </td>
                      <td style={{ color: parseFloat(bet.teamBProfit) >= 0 ? 'green' : 'red' }}>
                        {isOverMarketBet ? "0.00" : bet.teamBProfit || "0.00"}
                      </td>
                      <td>{bet.balance || "0.00"}</td>
                      <td>{isOverMarketBet ? "Over Market" : "Match Odds"}</td>
                      <td className={`exposure-cell ${bet.exposure >= 0 ? 'positive' : 'negative'}`}>
                        {isOverMarketBet ? 
                          (bet.type.toLowerCase() === 'yes' ? -bet.stake : -bet.teamAProfit).toFixed(2) :
                          bet.exposure.toFixed(2)}
                      </td>
                    </tr>
                  );
                })
              ...
              <tr>
                <td colSpan="10">No bets found</td>
              </tr>
            )}
          </tbody>
        </MyBetTable>
